  ...
  return (
    <TableRow>
      <TableCell className="font-mono text-xs text-muted-foreground">
        {subscription.id.replace("sub_p", "")}
      </TableCell>
      <TableCell>
        <div>
          <Link
            href={`/dashboard/admin/users/partners/${subscription.partnerId}`}
          ...
          <DropdownMenuContent align="end">
            ...
            {(subscription.status === "active" || subscription.status === "trialing") && (
              <>
                ...
                <DropdownMenuItem
                  onClick={() => console.log("View invoices:", subscription.partnerId)}
                >
                  <FileText className="mr-2 h-4 w-4" />
                  Invoice History
                </DropdownMenuItem>
                <DropdownMenuItem
                  onClick={() => console.log("View payment method:", subscription.partnerId)}
                >
            ...
            {subscription.status === "past_due" && (
              <>
                <DropdownMenuItem
                  onClick={() => console.log("Send reminder:", subscription.partnerId)}
            ...
            {subscription.status === "canceled" && (
              <DropdownMenuItem onClick={() => console.log("View invoices:", subscription.partnerId)}>
                <FileText className="mr-2 h-4 w-4" />
                Invoice History
              </DropdownMenuItem>
            )}
